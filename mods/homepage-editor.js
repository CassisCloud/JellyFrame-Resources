(function () {
    ...
    var State = {
        ...
        setApps: function(data) { 
            localStorage.setItem(STORAGE_KEY_APPS, JSON.stringify(data)); 
        },
    ...
    var Editor = {
        init: function() {
            if (document.getElementById('jf-edit-btn')) return;
            
            var header = document.querySelector('.jf-app-header');
            var appArea = document.getElementById(APP_AREA_ID);
            var target = document.querySelector(TARGET_SELECTOR);
            
            if (!header || !appArea || !target) return;

            this.injectCSS();
            this.injectEditButton(header, appArea, target);
            this.setupObservers(appArea, target);
            
            this.initDragAndDrop(appArea, '.app', function() { Editor.saveAppsLayout(appArea); });
            this.initDragAndDrop(target, 'div.verticalSection', function(cont) { Editor.saveSectionsLayout(cont); });
            
            ScrollEngine.init();

            this.applySectionsState(target);
            var apps = [].slice.call(appArea.querySelectorAll('.app'));
            for (var i = 0; i < apps.length; i++) {
                this.applyAppState(apps[i]);
                this.attachAppControls(apps[i], appArea);
            }
        },

        injectCSS: function() {
            if (document.getElementById('jf-layout-editor-styles')) return;
            var style = document.createElement('style');
            style.id = 'jf-layout-editor-styles';
            
            style.textContent = [
                ".jf-toggle-edit { background: rgba(20, 20, 20, 0.6); color: #fff; border: 1px solid rgba(255, 255, 255, 0.2); backdrop-filter: blur(10px); border-radius: 8px; padding: 8px 16px; font-size: 0.95rem; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 8px; transition: all 0.2s ease; position: relative; z-index: 10; }",
        ...
        injectEditButton: function(header, appArea, target) {
            ...
            editBtn.onclick = function() {
                ...
                if (!isEditMode) {
                    ...
                    self.saveSectionsLayout(target);
                }
            };
            header.appendChild(editBtn);
        },

        setupObservers: function(appArea, target) {
            var self = this;
                ...
                mutations.forEach(function(m) {
                    [].slice.call(m.addedNodes).forEach(function(node) {
                        if (node.nodeType === 1 && node.classList.contains('app')) {
                            self.applyAppState(node);
                            self.attachAppControls(node, appArea);
                            if (isEditMode) node.draggable = true;
                        }
                    });
                });
            });
            ...
            var targetObserver = new MutationObserver(function(mutations) {
                    ...
                    [].slice.call(m.addedNodes).forEach(function(node) {
                        if (node.nodeType === 1) {
                            if ((node.matches && node.matches('div.verticalSection')) || 
                                (node.querySelector && node.querySelector('div.verticalSection'))) {
                                needsApply = true;
                            }
                        }
                    });
                ...
                if (needsApply) {
                    self.applySectionsState(target);
                    if (isEditMode) {
                        var sections = [].slice.call(target.querySelectorAll('div.verticalSection'));
        ...
        saveAppsLayout: function(appArea) {
            var layout = {};
            var apps = [].slice.call(appArea.querySelectorAll(".app"));
            ...
            apps.forEach(function(app, index) {
                ...
                layout[app.id] = { order: index, col: match ? parseInt(match[1], 10) : 12 };
                app.setAttribute('data-order', index);
                app.style.order = index; 
            });
            
            State.setApps(layout);
        },

        applyAppState: function(app) {
            if (!app.id) return;
            var layout = State.getApps();
        ...
        saveSectionsLayout: function(target) {
            var layout = {};
            var sections = [].slice.call(target.querySelectorAll('div.verticalSection'));
            ...
            State.setSections(layout);
        },

        applySectionsState: function(target) {
            if (!target) return;
            target.classList.add('jf-reorderable'); 
            
            var layout = State.getSections();
            var sections = [].slice.call(target.querySelectorAll('div.verticalSection'));
        ...
        attachAppControls: function(app, appArea) {
        ...
        initDragAndDrop: function(container, itemSelector, onSave) {
    ...
    var ScrollEngine = {
        ...
        init: function() {
            ...
            document.addEventListener('dragend', function() { self.stop(); });

            document.addEventListener('wheel', function(e) {
                if (!isEditMode || !globalIsDragging) return;
                self.perform(e.deltaY);
            }, { passive: true });
        }
    };
    ...
    if (document.getElementById('jf-app-wrapper')) {
        Editor.init();
    }
